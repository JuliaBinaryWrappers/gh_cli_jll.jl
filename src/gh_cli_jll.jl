...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import gh_cli_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("gh_cli_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "gh_cli_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"gh_cli", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "gh_cli_jll", "src", "gh_cli_jll.jl"))
        @info("gh_cli_ll dev'ed out to /depot/dev/gh_cli_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("5d31d589-30fb-542f-b82d-10325e863e38"))
platforms = [Pkg.Artifacts.unpack_platform(e, "gh_cli", artifacts_toml) for e in artifacts["gh_cli"]]
...
if best_platform === nothing
    @debug("Unable to load gh_cli; unsupported platform $(triplet(platform_key_abi()))")
else
